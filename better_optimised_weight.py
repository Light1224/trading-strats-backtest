...
def simulate_trades(df, combined_score, buy_thresh, sell_thresh, initial_capital=100000):
    ...
    for i in range(len(df)):
        ...
        if position == 0 and score >= buy_thresh:
            ...
            cash = 0
            position = 1
        elif position == 1 and score <= sell_thresh:
            cash = shares * price
            shares = 0
            position = 0
...
if __name__=="__main__":
    ...
    print(f"Starting Bayesian optimization over {len(pbounds)} parameters...")
    ...
    optimizer.maximize(init_points=20, n_iter=80)  # 100 total iterations
    ...
    print(f"\nBest parameters found: Buy threshold = {buy_thresh:.3f}, Sell threshold = {sell_thresh:.3f}")
    all_stats = []
    all_portfolios = []
    for stock_file in stock_files:
    ...
    columns = ['Stock', 'Total Return', 'CAGR', 'Sharpe Ratio', 'Max Drawdown', 'Average Daily Return', 'Daily Return StdDev']
    summary_df = summary_df[columns].sort_values('Total Return', ascending=False)
    save_path = os.path.join(stats_folder, 'final_optimized_portfolio_stats.csv')
    summary_df.to_csv(save_path, index=False)
    print(f"\nSaved portfolio summary to {save_path}")
    print(summary_df.to_string(index=False))
    # Merge portfolios to include all available dates (full timeline)
    merged = None
    ...
    plt.grid(True)
    plt.show()
    daily_returns = np.diff(merged['Overall_Portfolio']) / merged['Overall_Portfolio'][:-1]
    plt.figure(figsize=(12,6))
