...
def moving_average_crossover_analysis(window_short=50, window_long=200):
    folder = 'data'
    save_folder = 'stats'
    ...
    for filename in os.listdir(folder):
        if filename.endswith('.csv'):
            ...
            # Convert 'Close' to numeric and remove invalid rows
    ...
    if results:
        ...
        save_path = os.path.join(save_folder, 'moving_average_crossover_stats.csv')
        df_summary.to_csv(save_path, index=False)
        print(f"\nSummary statistics saved to {save_path}")

if __name__ == "__main__":
    moving_average_crossover_analysis()
