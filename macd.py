...
def macd_strategy(data):
    ...
    data['Signal_Line'] = data['MACD'].ewm(span=9, adjust=False, min_periods=9).mean()
    ...
    for i in range(1, len(data)):
        if data.iloc[i]['MACD'] > data.iloc[i]['Signal_Line'] and data.iloc[i-1]['MACD'] <= data.iloc[i-1]['Signal_Line']:
            data.loc[data.index[i], 'Buy_Signal'] = data.iloc[i]['Close']
        elif data.iloc[i]['MACD'] < data.iloc[i]['Signal_Line'] and data.iloc[i-1]['MACD'] >= data.iloc[i-1]['Signal_Line']:
    ...
    stats = {
        "File": None,  # Will be set outside this function
...
if results:
    ...
    save_path = os.path.join(save_folder, 'macd_stats.csv')
