...
def rsi_strategy(data, rsi_period=14, oversold=30, overbought=70):
    ...
    for i in range(1, len(data)):
        ...
        if data.iloc[i]['RSI'] > oversold and data.iloc[i-1]['RSI'] <= oversold:
            data.loc[data.index[i], 'Buy_Signal'] = data.iloc[i]['Close']
        # Sell when RSI crosses below overbought threshold
        elif data.iloc[i]['RSI'] < overbought and data.iloc[i-1]['RSI'] >= overbought:
    ...
    stats = {
        "File": None,  # set outside function
...
if results:
    ...
    save_path = os.path.join(save_folder, 'rsi_stats.csv')
